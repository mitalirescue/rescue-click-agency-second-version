  ...
  return (
    <div className="w-full bg-slate-50 dark:bg-black/50 border-y border-slate-100 dark:border-white/5 py-12 transition-colors duration-300 overflow-hidden relative backdrop-blur-sm">
      ...
      <div className="relative w-full overflow-hidden group">
        <div className="flex animate-marquee hover:[animation-play-state:paused] items-center whitespace-nowrap">
          {marqueeTechs.map((tech, index) => (
            <div key={index} className="flex items-center space-x-3 mx-10 md:mx-16 opacity-40 hover:opacity-100 transition-opacity duration-300 cursor-pointer grayscale hover:grayscale-0 flex-shrink-0">
        ...
        <div className="absolute inset-y-0 left-0 w-32 bg-gradient-to-r from-slate-50 dark:from-black/50 to-transparent pointer-events-none z-10"></div>
        <div className="absolute inset-y-0 right-0 w-32 bg-gradient-to-l from-slate-50 dark:from-black/50 to-transparent pointer-events-none z-10"></div>
      </div>
    </div>
  );
};
